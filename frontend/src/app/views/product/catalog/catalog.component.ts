...
export class CatalogComponent implements OnInit {
                        ...
                        this.productService.getProducts(this.activeParams)
                            .subscribe(data => {
                                ...
                                if (this.cart && this.cart.items.length > 0) {
                                    this.products = data.items.map((product: ProductType): ProductType => {
                                        if (this.cart) {
    ...
    public removeAppliedFilter(appliedFilters: AppliedFilterType): void {
        if (appliedFilters.urlParam === 'heightFrom' || appliedFilters.urlParam === 'heightTo' ||
            appliedFilters.urlParam === 'diameterFrom' || appliedFilters.urlParam === 'diameterTo') {
            delete this.activeParams[appliedFilters.urlParam];
            this.activeParams.page = 1;
        } else {
            this.activeParams.types = this.activeParams.types.filter(item => item !== appliedFilters.urlParam);
        }
